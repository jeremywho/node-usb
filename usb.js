...
Object.defineProperty(usb.Device.prototype, "configDescriptor", {
  get: function () {
    try {
      return (
        this._configDescriptor ||
        (this._configDescriptor = this.__getConfigDescriptor())
      );
    } catch (e) {
...
Object.defineProperty(usb.Device.prototype, "allConfigDescriptors", {
  get: function () {
    try {
      return (
        this._allConfigDescriptors ||
        (this._allConfigDescriptors = this.__getAllConfigDescriptors())
      );
    } catch (e) {
...
usb.Device.prototype.controlTransfer = function (
  ...
  if (isIn) {
    if (!(data_or_length >= 0)) {
      throw new TypeError(
        "Expected size number for IN transfer (based on bmRequestType)"
      );
    }
    ...
    if (!isBuffer(data_or_length)) {
      throw new TypeError(
        "Expected buffer for OUT transfer (based on bmRequestType)"
      );
    }
      ...
      if (callback) {
        if (isIn) {
          callback.call(
            self,
            error,
            buf.slice(SETUP_SIZE, SETUP_SIZE + actual)
          );
        } else {
      ...
      this.controlTransfer(
        ...
        function (error, buffer) {
          ...
          while (i < descriptor.wTotalLength) {
            ...
            capability.dev_capability_data = buffer.slice(
              i + 3,
...
usb.Device.prototype.setConfiguration = function (desired, cb) {
  ...
  this.__setConfiguration(desired, function (err) {
    if (!err) {
      this.interfaces = [];
      var len = this.configDescriptor
        ? this.configDescriptor.interfaces.length
        : 0;
      for (var i = 0; i < len; i++) {
        this.interfaces[i] = new Interface(this, i);
      }
...
Interface.prototype.__refresh = function () {
  this.descriptor =
    this.device.configDescriptor.interfaces[this.id][this.altSetting];
  ...
  for (var i = 0; i < len; i++) {
    ...
    var c =
      desc.bEndpointAddress & usb.LIBUSB_ENDPOINT_IN ? InEndpoint : OutEndpoint;
...
InEndpoint.prototype.startPoll = function (nTransfers, transferSize) {
  var self = this;
  this.pollTransfers = InEndpoint.super_.prototype.startPoll.call(
    this,
    nTransfers,
    transferSize,
    transferDone
